...
namespace Micosmo.SensorToolkit {
    ...
    public class SignalFilter {
        ...
        public bool TestCollider(Collider col) => IsPass(col.gameObject, col.attachedRigidbody?.gameObject);
        public bool TestCollider(Collider2D col) => IsPass(col.gameObject, col.attachedRigidbody?.gameObject);

        bool IsPass(GameObject go, GameObject rbGo) {
            if (IsNull()) {
                return true;
            }
            ...
            return IsPassingTagFilter(go) || (rbGo != null && IsPassingTagFilter(rbGo));
        }
