...
namespace Micosmo.SensorToolkit {
    ...
    public class LocomotionSystem {
        ...
        void FlyableSeekWithForces(Rigidbody rb, Vector3 tpos, Vector3 tdir, Vector3 tup) {
            var angularAccel = MotionUtils.SeekAngularAccel(springForce, dampForce, MaxAngularAccelDegrees, new ReferenceFrame(rb.transform), Mathf.Rad2Deg * rb.angularVelocity, tdir, tup);
        ...
        void FlyableSeekKinematic(Rigidbody rb, Vector3 tpos, Vector3 tdir, Vector3 tup) {
            var angularAccel = MotionUtils.SeekAngularAccel(springForce, dampForce, MaxAngularAccelDegrees, new ReferenceFrame(rb.transform), rb.rotation*kinematicAngularVelocity, tdir, tup);
        ...
        void CharacterSeekWithForces(Rigidbody rb, Vector3 tpos, Vector3 tdir, Vector3 tup) {
            var angularAccel = MotionUtils.SeekPlanarAngularAccel(springForce, dampForce, MaxAngularAccelDegrees, new ReferenceFrame(rb.transform), Mathf.Rad2Deg * rb.angularVelocity, tdir, tup);
            var transAccel = MotionUtils.SeekAccel(springForce, dampForce, rb.position - tpos, rb.velocity);
            AccelerateForces(rb, angularAccel, transAccel);
        }

        void CharacterSeekKinematic(Rigidbody rb, Vector3 tpos, Vector3 tdir, Vector3 tup) {
            var angularAccel = MotionUtils.SeekPlanarAngularAccel(springForce, dampForce, MaxAngularAccelDegrees, new ReferenceFrame(rb.transform), rb.rotation * kinematicAngularVelocity, tdir, tup);
            var transAccel = MotionUtils.SeekAccel(springForce, dampForce, rb.position - tpos, kinematicVelocity);
            AccelerateKinematic(rb, angularAccel, transAccel);
        }

        void CharacterControllerSeek(CharacterController cc, Vector3 tpos, Vector3 tdir, Vector3 tup) {
            var angularAccel = MotionUtils.SeekPlanarAngularAccel(springForce, dampForce, MaxAngularAccelDegrees, new ReferenceFrame(cc.transform), cc.transform.rotation * kinematicAngularVelocity, tdir, tup);
        ...
        void RigidBody2DSeekWithForces(Rigidbody2D rb, Vector2 tpos, Vector2 tdir) {
            var angularAccel = MotionUtils.SeekAngularAccel2D(springForce, dampForce, MaxAngularAccelDegrees, rb.transform.up, rb.angularVelocity, tdir);
            var transAccel = MotionUtils.SeekAccel(springForce, dampForce, rb.position - tpos, rb.velocity);
            AccelerateForces(rb, angularAccel, transAccel);
        }

        void RigidBody2DSeekKinematic(Rigidbody2D rb, Vector2 tpos, Vector2 tdir) {
            var angularAccel = MotionUtils.SeekAngularAccel2D(springForce, dampForce, MaxAngularAccelDegrees, rb.transform.up, kinematicAngularVelocity2D, tdir);
