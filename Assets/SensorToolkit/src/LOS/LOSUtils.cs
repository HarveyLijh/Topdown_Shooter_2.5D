...
namespace Micosmo.SensorToolkit {
    ...
    public class LOSUtils {
        ...
        public static Vector3 GetRandomPointInTriangles(List<Triangle> triangles) {
            ...
            var r = Random.Range(0f, totalArea);
            for (int i = 0; i < triangleAreas.Count; i++) {
                if (triangleAreas[i] >= r) {
                    return triangles[i].GetRandomPoint();
                }
            }
            return triangles[triangles.Count - 1].GetRandomPoint();
        }

        static List<float> edgeLengths = new List<float>();
        public static Vector3 GetRandomPointOnEdges(List<Edge2D> edges) {
            if (edges.Count == 0) {
                return default;
            }
            ...
            var r = Random.Range(0f, totalLength);
            for (int i = 0; i < edgeLengths.Count; i++) {
                if (edgeLengths[i] >= r) {
                    return edges[i].GetRandomPoint();
                }
            }
            return edges[edges.Count - 1].GetRandomPoint();
        }
