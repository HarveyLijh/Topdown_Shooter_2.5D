    ...
    public class SoldierAI : StupidSimpleFSM {
        ...
        IEnumerator DefaultState() {
            while (true) {
                ...
                while (countdown > 0f) {
                    ...
                    SteerSensor.Seek.Destination = Vector3.zero;
        ...
        IEnumerator ChargeState(GameObject target) {
            ...
            while (timer >= 0f) {
                ...
                SteerSensor.Seek.DestinationTransform = target.transform;
                SteerSensor.Locomotion.Strafing.Clear();
        ...
        IEnumerator StrafeState(GameObject target) {
            ...
            while (timer > 0f) {
                ...
                SteerSensor.Seek.Destination = transform.position + strafeDirection * 10;
        ...
        IEnumerator FleeState() {
            ...
            SteerSensor.Locomotion.Strafing.Clear();
            SteerSensor.Seek.TargetDistance = 20f; // Seek position 20-units away from target. This will cause flee
            ...
            while (timer > 0) {
                ...
                SteerSensor.Seek.DestinationTransform = nearestEnemy.transform;
            }

            SteerSensor.Seek.ClearDestination();
            SteerSensor.Seek.TargetDistance = 0f;
        ...
        IEnumerator PickUpState(Holdable pickup) {
            ...
            while (timer > 0f) {
                ...
                SteerSensor.Seek.DestinationTransform = pickup.transform;
                SteerSensor.Locomotion.Strafing.Clear();
        ...
        IEnumerator CarryToBaseState() {
            while (true) {
                ...
                SteerSensor.Seek.DestinationTransform = myBase.transform;
                SteerSensor.Locomotion.Strafing.Clear();
            }
        }
    }
