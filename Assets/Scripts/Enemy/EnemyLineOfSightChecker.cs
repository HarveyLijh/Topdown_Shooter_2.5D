...
public class EnemyLineOfSightChecker : MonoBehaviour
{
    ...
    private bool CheckLineOfSight(Transform Target)
    {
        ...
        if (dotProduct >= Mathf.Cos(FieldOfView/2))
        {
    ...
    public bool CheckDirectLineOfSight(Transform Target)
    {
        Vector3 direction = (Target.transform.position - transform.position).normalized;
        if (Physics.Raycast(transform.position, direction, out RaycastHit hit, Collider.radius,LayerMask.GetMask("Obstacles")))
        {
            return true;
        }
        return false;
