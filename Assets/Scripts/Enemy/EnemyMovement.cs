...
public class EnemyMovement : MonoBehaviour
{
    ...
    private IEnumerator Hide(Transform Target)
    {
        ...
        while (true)
        {
            ...
            for (int i = 0; i < hits; i++)
            {
                if (NavMesh.SamplePosition(Colliders[i].transform.position, out NavMeshHit hit, Agent.height*2, Agent.areaMask))
                {
                    ...
                    else
                    {
                        ...
                        if (NavMesh.SamplePosition(Colliders[i].transform.position - (Target.position - hit.position).normalized * 2, out NavMeshHit hit2, Agent.height * 2, Agent.areaMask))
                        {
